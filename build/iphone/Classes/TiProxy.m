...
//Rare exceptions to indicate a bug in the _aiai code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_aiai = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_aiai]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.aiai.module"
